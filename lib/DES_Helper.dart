...
class DES {
  ...
  List<int> crypt(List<int> data, int cryptType) {
    ...
    if (_baseDES.mode == DESMode.CBC || _baseDES.mode==DESMode.OFB) {
      if (_baseDES.iv != null) {
    ...
    while (i < data.length) {
      ...
      //TODO: This is the added code:
      else if (_baseDES.mode == DESMode.OFB) {
        List<int> preoutput;
        if (cryptType == _ENCRYPT) {
          preoutput = _desCrypt(iv, cryptType);
          iv=preoutput;
          processedBlock = xor(preoutput, block);
        }
         //_desCrypt(block, cryptType);
        if (cryptType == _DECRYPT) {
          preoutput=_desCrypt(iv, cryptType);
          processedBlock = xor(preoutput, block);
          iv = preoutput;
